...
data = [[dict({'scenario':scenario},**{backend:'0' for backend in backends}) for scenario in scenarios] for _ in range(len(frame_names))]
for dir_idx, dirname in enumerate(scenarios):
    for backend in backends:
        try:
            res = get_data(os.path.join(cwd, dirname, f'out_{backend}'))
            for idx, val in enumerate(res):
                data[idx][dir_idx]['scenario'] = dirname
                data[idx][dir_idx][backend] = val
        except:
            pass

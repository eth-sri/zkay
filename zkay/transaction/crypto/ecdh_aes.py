...
class EcdhAesCrypto(ZkayCryptoInterface):
    ...
    def _gen_keypair():
        ...
        keys, _ = run_command(['java', '-Xms4096m', '-Xmx16384m', '-cp', f'{circuit_builder_jar}', 'zkay.ZkayECDHGenerator', rnd])
    ...
    def _generate_or_load_key_pair(self, address: str) -> KeyPair:
        # TODO store and load keys instead of regenerating every time
        pk, sk = self._gen_keypair()
    ...
    def _dec(self, cipher: Tuple[int, ...], my_sk: Any) -> Tuple[int, List[int]]:
        ...
        sender_pk = self.keystore.getPk(cipher[-1])
        cipher = cipher[:-1]
